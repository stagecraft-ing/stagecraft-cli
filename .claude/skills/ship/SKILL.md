...
The gate is the installed `spec-spine` binary on your `PATH`, plus the cargo
gates once spec 002 lands. If `spec-spine` is missing, run `/setup`.
...
- `git branch --show-current`. If on the default branch, STOP and create a
  ...
  Never commit straight to the default branch. (The AGENTS.md backlog protocol
  lets a completed single-spec session push to main directly; /ship is the PR
  path, and PRs come from feature branches.)
...
`.derived/` is committed in this repo and the binary regenerates it
...
```sh
spec-spine compile                       # specs -> the registry shards
...
Once spec 002 lands and code exists, the cargo gates follow, in order:
...
- `index check` reports stale (exit 2): run `spec-spine index` to regenerate,
  then stage and commit the regenerated `.derived/` shards with your change;
  CI runs the same staleness gate.
...
- A cargo gate fails: fix the code; never weaken a guard, loosen a lint, or
  soften a test to get past it.

## Step 2: review the diff

Invoke the `code-review` skill on the working diff. Apply confirmed, actionable
fixes. If a fix touches any gate input (a `spec.md`, `Cargo.toml`, a schema, a
workflow, the kit under `.claude/`), re-run Step 1 before continuing.

## Step 3: commit

Invoke the `commit` skill (conventional, impact-focused message, spec id as
scope when the work belongs to one) on a feature branch.
...
- CI re-runs the same gate (`compile` / `index check` / `lint --fail-on-warn` /
  `couple`) on the PR via `.github/workflows/spec-spine.yml`. A local pass
  should mean a clean CI run; if CI still fails on a gate the local run passed,
  halt and present the divergence (orchestrator rule: halt on failure).
