...
summary: >
  ...
  binary named stagecraft that exposes the platform's governance verbs
  ...
  (Claude Code first) operates natively under Stagecraft governance,
  requesting approvals, checking spec-code coupling, and triggering
  factory stages. This is milestone M4 in the Stagecraft ladder; the spec
...
- **CLI face** (humans): governance verbs as subcommands: authenticate
  against a Stagecraft control plane, inspect tenants and fleets, request
...
- **MCP face** (agents): the same verbs exposed as MCP tools over stdio,
  so a coding agent operates under Stagecraft governance natively instead
...
- **Binary name `stagecraft`**; npm distribution as `stagecraft-cli` (the
...
the Stagecraft milestone ladder; see stagecraft spec 001 §6). Verb
...
- Control-plane behavior (stagecraft repo).
